  ...
  async prepare(): Promise<void> {
    this.config = await Promise.all(
      ...
      this.config.map(async item => {
        if (item.serverType === 'swagger' || item.serverType === 'openapiv3') {
          const swaggerToYApiServer = new SwaggerToYApiServer({
            swaggerJsonUrl: item.serverUrl,
            openapiv3: item.serverType === 'openapiv3',
          })
        ...
        if (item.serverUrl) {
          // 去除地址后面的 /
          // fix: https://github.com/fjc0k/@bfr/ytt/issues/22
        ...
        if (!syntheticalConfig.typesOnly) {
          if (!(await fs.pathExists(rawRequestFunctionFilePath))) {
            await fs.outputFile(
              requestFunctionFilePath,
              dedent`
                import type { RequestFunctionParams } from '@bfr/ytt'
          ...
          if (
            ...
            await fs.outputFile(
              ...
              dedent`
                ...
                import type { RequestConfig } from '@bfr/ytt'
